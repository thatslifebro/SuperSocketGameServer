...
public class Chess
{
    ...
    public void CheckMovePiece(PKTReqMovePiece request, User player, bool otherPlayerTurn, Turn turn)
    {
        ...
        var whiteTurn = turn == Turn.White ? true : false;
        PossibleMovesByPieces(tableClass.Table[BeforeMove_I, BeforeMove_J], BeforeMove_I, BeforeMove_J, whiteTurn, otherPlayerTurn);
        ...
        var isUserWhitePlayer = (whiteTurn && !otherPlayerTurn) || (!whiteTurn && otherPlayerTurn) ? true : false;
        CastlingAndPawnPromotionChecker(ax, ay, (int)request.Promotion, isUserWhitePlayer);
    ...
    public void PossibleMovesByPieces(int x, int i, int j, bool WhiteTurn, bool OtherPlayerTurn)
    {
        ...
        RemoveMoveThatNotPossible(x, i, j, WhiteTurn);
    }

    public void RemoveMoveThatNotPossible(int x, int a, int b, bool WhiteTurn)
    {
        for (int i = 0; i < 8; i++)
        {
            for (int j = 0; j < 8; j++)
    ...
    public void EveryPossibleMoves(Turn turn)
    {
        var WhiteTurn = turn == Turn.White ? true : false;
        var OtherPlayerTurn = ((WhiteTurn && turn == Turn.Black) || (!WhiteTurn && turn == Turn.White)) ? true : false;
            ...
            for (i = 0; i < 8; i++)
            {
                for (j = 0; j < 8; j++)
                {
                    if (tableClass.Table[i, j] == x)
                    {
                        ...
                        RemoveMoveThatNotPossible2(x, i, j, WhiteTurn);
                    }
                }
            }
        }
    ...
    public void RemoveMoveThatNotPossible2(int x, int a, int b, bool WhiteTurn)
    {
    ...
    public void CastlingAndPawnPromotionChecker(int i, int j, int promotionValue, bool isUserWhitePlayer)
    {
